...
!(async () => {
  ...
  if (ids == "") {
    $notify("Tất cả các TF đã được thêm vào", "Vui lòng đóng thủ công", "");
    $done();
  } else {
    ids = ids.split(",");
    try {
    ...
    $task.fetch({ url: testurl + ID, method: "GET", headers: header }).then(
      (resp) => {
        ...
        if (resp.status == 404) {
          ...
          console.log(ID + " " + "TF không tồn tại，APP_ID đã tự động bị xóa");
          $notify(ID, "TF không tồn tại", "APP_ID đã tự động bị xóa");
          resolve();
        } else {
